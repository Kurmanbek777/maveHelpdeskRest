...
// Model for Department entity
    models.Department = Backbone.Model.extend({
        urlRoot: "http://localhost:2822/maveHelpdeskRest/webresources/com.test.mavehelpdeskrest.department/",
        idAttribute: 'id',
        defaults: {
            department: ""
        },
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:2822/maveHelpdeskRest/webresources/com.test.mavehelpdeskrest.department/';
            }
    ...
    // Collection class for Department entities
    models.DepartmentCollection = Backbone.Collection.extend({
        model: models.Department,
        url: "http://localhost:2822/maveHelpdeskRest/webresources/com.test.mavehelpdeskrest.department/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.DepartmentCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-department-list-itemis template identifier for item
                        templateName: '#tpl-department-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.department = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    ...
                    // tpl-department-details is template identifier for chosen model element
                    templateName: '#tpl-department-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Department();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-department-details is a template identifier for chosen model element
                templateName: '#tpl-department-details',
